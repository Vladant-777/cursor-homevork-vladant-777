...
const getBestStudent = (students) =>
  students.sort((a, b) => getAverageMark(a) - getAverageMark(b))[
    students.length - 1
  ].name;
...
const calculateWordLetters = (words) =>
  words
    .toLowerCase()
    .split('')
    .reduce((total, letter, i, arr) => {
      total[letter] = arr.filter(
        (letterInArr) => letterInArr === letter
      ).length;
      return total;
    }, {});
console.log(calculateWordLetters(`dnfkjsnadkjfkjasndkf`));
